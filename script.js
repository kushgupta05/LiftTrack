...
function renderStructuredWorkout() {
  ...
  container.innerHTML = `<div class="structured-progress panel"><div><p class="eyebrow">${escapeHTML(currentWorkout.planName || "PLAN WORKOUT")}</p><h2>${completed} / ${total} sets completed</h2></div><strong>${percent}%</strong><div class="structured-progress-track"><span style="width:${percent}%"></span></div></div>${planExercises.map((item, exerciseIndex) => {
    ...
    return `<article class="structured-exercise panel${exerciseCompleted === item.targetSets ? " exercise-complete" : ""}"><header><div><p class="eyebrow">EXERCISE ${exerciseIndex + 1}</p><h3>${exerciseCompleted === item.targetSets ? "✓ " : ""}${escapeHTML(item.exercise)}</h3><button class="form-guide-link" data-open-guide="${encodeURIComponent(item.exercise)}">${getFormGuide(item.exercise) ? "View form guide" : "Detailed form guide coming soon"}</button></div><strong>${exerciseCompleted} / ${item.targetSets} sets</strong></header>${previous}${slots}</article>`;
  }).join("")}`;
}
...
async function addFood() {
  ...
  showToast(refreshed ? "Protein progress updated." : "Food was saved, but nutrition data could not be refreshed.", refreshed ? "success" : "error");
}
...
function initializePWA() { updateNetworkStatus(); window.addEventListener("online", updateNetworkStatus); window.addEventListener("offline", updateNetworkStatus); if ("serviceWorker" in navigator && ["http:", "https:"].includes(location.protocol)) { let refreshing = false; navigator.serviceWorker.addEventListener("controllerchange", () => { if (refreshing) return; refreshing = true; location.reload(); }); navigator.serviceWorker.register("/service-worker.js", { scope: "/" }).then(registration => registration.update()).catch(error => console.warn("Service worker registration failed:", error)); } }
